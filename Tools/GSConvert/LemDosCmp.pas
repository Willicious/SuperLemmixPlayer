...
type
  TDosDatSectionListEx = class(TObjectList)
  private
    function GetItem(Index: Integer): TDosDatSectionEx;
    procedure SetItem(Index: Integer; const Value: TDosDatSectionEx);
  protected
  public
    function Add(Item: TDosDatSectionEx): Integer;
    procedure Insert(Index: Integer; Item: TDosDatSectionEx);
    property Items[Index: Integer]: TDosDatSectionEx read GetItem write SetItem; default;
  end;
...
type
  ...
  { higher }
    function LoadSection(SrcStream: TStream; Sec: TDosDatSectionEx; DecompressOnTheFly: Boolean = True): Boolean;
    procedure LoadSectionList(SrcStream: TStream; aList: TDosDatSectionListEx; DecompressOnTheFly: Boolean = True);
...
type
  ...
  private
    DSize       : Integer;
    CSize       : Integer;
    di          : Integer; // index of source data byte
    ci          : Integer; // index of dest data byte
    BitCnt      : Byte;
    CData       : PBytes;
    DData       : PBytes;
...
{-------------------------------------------------------------------------------
  ...
  There can be more sections in a file of course: use method DecompressSections
  for that.
...
function TDosDatDecompressor.LoadSection(SrcStream: TStream;
  Sec: TDosDatSectionEx; DecompressOnTheFly: Boolean): Boolean;
var
  Header: TCompressionHeaderRec;
  Rd: Integer;
  CopySize: Integer;
...
procedure TDosDatDecompressor.LoadSectionList(SrcStream: TStream; aList: TDosDatSectionListEx;
...
constructor TDosDatSectionEx.Create;
begin
  inherited;
...
destructor TDosDatSectionEx.Destroy;
begin
...
{ TDosDatSectionListEx }

function TDosDatSectionListEx.Add(Item: TDosDatSectionEx): Integer;
begin
  Result := inherited Add(Item);
end;

function TDosDatSectionListEx.GetItem(Index: Integer): TDosDatSectionEx;
begin
  Result := inherited Get(Index);
end;

procedure TDosDatSectionListEx.Insert(Index: Integer; Item: TDosDatSectionEx);
begin
  inherited Insert(Index, Item);
end;

procedure TDosDatSectionListEx.SetItem(Index: Integer;
  const Value: TDosDatSectionEx);
begin
...
begin
  ...
  try
    ...
    try
      ...
      Header.DecompressedSize := (DSize + 1) mod 65536;
