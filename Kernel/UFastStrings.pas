...
(*{function FastReplace(const aSourceString : String; const aFindString, aReplaceString : String;
   ReplaceFlags: TReplaceFlags{CaseSensitive : Boolean = False}) : String;} *)
...
begin
  ...
  //Assert(StartPos > 0);{#EL}

  Result := 0;
  if StartPos < 1 then Exit; {#EL}
  L := Length(aSource);
  if L = 0 then exit;
...
begin
  ...
  //Assert(StartPos > 0);
  Result := 0; {#EL}
  if (aSourceLen < 1) or (aFindLen < 1) or (StartPos < 1) then Exit; {#EL}


(*  if StartPos <= 0 then {#EL}
  begin
    Result := 0;
    Exit;
  end; *)
...
//This is because I will just be passed a pointer to the data        sysutils
